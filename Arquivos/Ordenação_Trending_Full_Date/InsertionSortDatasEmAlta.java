...
public class InsertionSortDatasEmAlta {
    ...
    // Função para ordenar os dados pela data completa usando Insertion Sort
    private static void insertionSort(List<String> linhas, Comparator<String> comparator) {
        for (int i = 1; i < linhas.size(); i++) {
            String chave = linhas.get(i);
            int j = i - 1;
            while (j >= 0 && comparator.compare(linhas.get(j), chave) > 0) {
                linhas.set(j + 1, linhas.get(j));
                j = j - 1;
            }
            linhas.set(j + 1, chave);
        }
    }

    // Função para gerar o caso melhor
    private static void gerarMelhorCaso(List<String> linhas) throws IOException {
        // Ordenar os dados pela data completa em ordem decrescente usando Insertion Sort
        ...
        insertionSort(linhasOrdenadas, comparator);
        // Escrever os dados ordenados em um novo arquivo CSV
        escreverCSV("videos_T1_trending_full_date_insertionSort_melhorCaso.csv", linhasOrdenadas);
    }
    ...
    private static void gerarCasoMedio(List<String> linhas) throws IOException {
        ...
        insertionSort(linhasEmbaralhadas, comparator);
        // Escrever os dados ordenados em um novo arquivo CSV
        escreverCSV("videos_T1_trending_full_date_insertionSort_casoMedio.csv", linhasEmbaralhadas);
    }

    // Função para gerar o caso pior
    private static void gerarPiorCaso(List<String> linhas) throws IOException {
        // Ordenar os dados pela data completa em ordem crescente usando Insertion Sort
        Comparator<String> comparator = new Comparator<String>() {
            SimpleDateFormat dateFormat = new SimpleDateFormat("yy.dd.MM");
        ...
        insertionSort(linhas, comparator);

        // Escrever os dados ordenados em um novo arquivo CSV
        escreverCSV("videos_T1_trending_full_date_insertionSort_piorCaso.csv", linhas);
    }
}
