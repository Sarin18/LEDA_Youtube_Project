...
public class QuickSortDatasEmAlta {
    ...
    // Função para ordenar os dados pela data completa usando Quick Sort
    private static void quickSort(List<String> linhas, Comparator<String> comparator) {
        quickSort(linhas, 0, linhas.size() - 1, comparator);
    }

    private static void quickSort(List<String> linhas, int inicio, int fim, Comparator<String> comparator) {
        if (inicio < fim) {
            int indicePivo = particionar(linhas, inicio, fim, comparator);
            quickSort(linhas, inicio, indicePivo - 1, comparator);
            quickSort(linhas, indicePivo + 1, fim, comparator);
        }
    }

    private static int particionar(List<String> linhas, int inicio, int fim, Comparator<String> comparator) {
    ...
    private static void gerarMelhorCaso(List<String> linhas) throws IOException {
        // Ordenar os dados pela data completa em ordem decrescente usando Quick Sort
        ...
        quickSort(linhasOrdenadas, comparator);
        // Escrever os dados ordenados em um novo arquivo CSV
        escreverCSV("videos_T1_trending_full_date_quickSort_melhorCaso.csv", linhasOrdenadas);
    }
    ...
    private static void gerarCasoMedio(List<String> linhas) throws IOException {
        ...
        quickSort(linhasEmbaralhadas, comparator);
        // Escrever os dados ordenados em um novo arquivo CSV
        escreverCSV("videos_T1_trending_full_date_quickSort_casoMedio.csv", linhasEmbaralhadas);
    }

    // Função para gerar o caso pior
    private static void gerarPiorCaso(List<String> linhas) throws IOException {
        // Ordenar os dados pela data completa em ordem crescente usando Quick Sort
        Comparator<String> comparator = new Comparator<String>() {
            SimpleDateFormat dateFormat = new SimpleDateFormat("yy.dd.MM");
        ...
        quickSort(linhas, comparator);
        // Escrever os dados ordenados em um novo arquivo CSV
        escreverCSV("videos_T1_trending_full_date_quickSort_piorCaso.csv", linhas);
    }
}
