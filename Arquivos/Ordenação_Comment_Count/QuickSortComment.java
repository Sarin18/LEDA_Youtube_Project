...
public class QuickSortComment {
    ...
    // Função para ordenar os dados pelo número de comentários usando Quick Sort
    private static void quickSort(List<String> linhas, int inicio, int fim) {
        if (inicio < fim) {
            int indicePivo = particionar(linhas, inicio, fim);
            quickSort(linhas, inicio, indicePivo - 1);
            quickSort(linhas, indicePivo + 1, fim);
        }
    }

    // Função para particionar a lista e retornar o índice do pivô
    private static int particionar(List<String> linhas, int inicio, int fim) {
        int pivo = getQuantidadeComentarios(linhas.get(fim));
        int i = inicio - 1;
        for (int j = inicio; j < fim; j++) {
            if (getQuantidadeComentarios(linhas.get(j)) <= pivo) {
                i++;
    ...
    private static void gerarMelhorCaso(List<String> linhas) throws IOException {
        ...
        quickSort(linhasOrdenadas, 0, linhasOrdenadas.size() - 1);
